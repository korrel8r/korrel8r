...
That is something we have to live with while we wait and hope for "one standard to rule them all".
...
<<_domain_k8s>> has a class for each resource Kind; `Pod.k8s`, `DaemonSet.k8s` and so on.
