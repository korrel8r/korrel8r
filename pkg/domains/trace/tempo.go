...
func (r *tracesSpan) UnmarshalJSON(data []byte) error {
	// rawSpan has the same fields as tracesSpan but no methods,
	// so json.Unmarshal uses the struct tags (camelCase) without recursion.
	type rawSpan tracesSpan
	var raw rawSpan
	if err := json.Unmarshal(data, &raw); err == nil && len(raw.TraceID) > 0 {
		*r = tracesSpan(raw)
		return nil
	}
	// Fallback to snake_case (OTLP spec format).
	var snake struct {
