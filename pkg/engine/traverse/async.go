...
type asyncTraverser struct{ traverser }
...
func NewAsync(e *engine.Engine, g *graph.Graph, start korrel8r.Class, objects []korrel8r.Object, queries []korrel8r.Query) Traverser {
	return &asyncTraverser{*newTraverser(e, g, start, objects, queries)}
}
...
func (t *asyncTraverser) ensureNode(n *graph.Node) *node {
	if n.Value == nil {
		n.Value = &node{Node: n, asyncTraverser: t, queryChan: make(chan lineQuery)}
	}
	return getNode(n)
}
...
func (n *node) Run(ctx context.Context) {
	...
	for lq := range n.queryChan {
		...
		log.V(4).Info("Query begin", "q", q) // FIXME
		count := 0
		...
		log.V(4).Info("Query end", "q", q, "n", count)
