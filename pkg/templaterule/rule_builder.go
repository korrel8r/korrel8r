...
func newRuleBuilder(r *Rule, domains map[string]korrel8r.Domain, funcs template.FuncMap) (*ruleBuilder, error) {
	var (
		err error
		rb  = &ruleBuilder{name: r.Name, domains: domains, funcs: funcs}
	)
	if rb.name == "" {
...
func (rb *ruleBuilder) expand(spec *ClassSpec, what string) (classes []korrel8r.Class, err error) {
	domain, ok := rb.domains[spec.Domain]
	if !ok {
		return nil, fmt.Errorf("unknown domain: %v", domain)
	}
