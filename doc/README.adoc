...
The start and goal can be in different domains.
Rules are implemented as {go-template} that use data from the _start object_
and to create  a _query_ in the _goal domain_.

Rules are bridge between domains different vocabularies, schema, naming conventions and query languages.
...
- Some domains provide additional functions, see <<_about_domains>>.
