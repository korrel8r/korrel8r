...
You can configure the set of stores Korrel8r connects to, and modify or extend the correlation rules it uses, via a configuration file. Configuration files can be in either JSON or YAML format.
...
<7> Specify a short name for a group of classes. Creating a group is optional, but if you create a group you must specify a name.
<8> Specify the domain for classes that are included in the group. All classes included in a group must be in the same domain.
<9> Specify a list of classes to include in the group.
...
<10> Specify a short, descriptive name for the store.
...
<12> Specify the URL for the store.
<13> Optional: Specify a list of paths to additional configuration files to include.
