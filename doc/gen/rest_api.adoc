...
| PUT | /api/v1alpha1/config | link:#id-put-config[put config] | Set verbose level for logging on a running server.
...
[id=id-put-config]
=== Set verbose level for logging on a running server.
