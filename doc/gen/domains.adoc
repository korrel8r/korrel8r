...
Selector is a https://grafana.com/docs/loki/latest/query/[LogQL] query string. Examples:
...
Represents a https://opentelemetry.io/docs/concepts/signals/traces/#spans[span]. A trace is a set of spans with the same trace-id, there is no explicit class representing a trace.
...
A https://grafana.com/docs/tempo/latest/traceql/[TraceQL] query can select spans from many traces. Example:
