...
- observed\_timestamp: time the log was first stored or recorded, in RFC3339 format.

### Viaq and OTEL attributes

The openshift logging collector can store logs in two formats:

- Viaq: legacy format with attributes like: "kubernetes\_namespace\_name", "kubernetes\_pod\_name"
- OTEL: new standard format with attributes like: "k8s\_namespace\_name", "k8s\_pod\_name". Note the use of "\_" rather than "." to give legal Loki label names. Otherwise these names are as given in the OTEL specification.
