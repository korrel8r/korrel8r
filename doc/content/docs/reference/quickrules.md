...
- Use [quicktemplate](<https://github.com/valyala/quicktemplate>) syntax; \{% code %\} blocks run type\-checked Go.
...
\{% func RuleName\(o interface\{\}\) %\} receives the start object. Use a type assertion in a \{% code %\} block to access domain\-specific fields.
...
- [Fail](<#Empty>) \-\- panics with a formatted message, aborting rule execution.
- A blank \(whitespace\-only\) output means the rule does not apply.

### Error Handling

Rule execution recovers from panics, so panic is the correct way to signal that a rule cannot apply. Use [Fail](<#Empty>) for an explicit message, or let Go's normal runtime panics \(nil dereference, bad type assertion, etc.\) abort execution. There is no need to defensively check for these errors when the intended outcome is simply to skip the rule.

Helper functions for error handling:

- [Fail](<#Empty>)\(format, args...\) \-\- panic with a formatted error.
- [Require](<#Require>)\(v\) \-\- return v if non\-empty, panic otherwise.
- [RequireAll](<#Empty>)\(values...\) \-\- panic if any value is empty.
- [Empty](<#Empty>)\(v\) \-\- true if v is nil, false, zero, or zero\-length.
- [Default](<#Default>)\(dflt, v\) \-\- return v if non\-empty, else dflt.
- [ToJSON](<#Empty>)\(v\) \-\- serialize v to JSON, panic on error.

### Imports

Templates may import Go use inside \{% code %\} blocks. Rule templates typically import the start\-object's domain package \(for type assertions\) and pkg/rules \(for [Fail](<#Empty>)\):
...
The bare text between two template\-tag blocks is a YAML descriptor for the following rule, using the same schema as a configuration\-file rule \(see [github.com/korrel8r/korrel8r/pkg/config](<https://pkg.go.dev/github.com/korrel8r/korrel8r/pkg/config/>)\). At startup [Rules](<#Rules>) parses the annotations from the embedded \*.qtpl source and verifies that:
...
- doc.go \-\- \(this file\).
- helpers.go, k8s\_helpers.go \-\- Shared helper functions for templates.
...
Add table\-driven cases to quickrules\_test.go \(a start object in, the expected goal query strings out\):
...
[Rules](<#Rules>) returns the pre\-compiled rules for a set of domains. The korrel8r CLI adds them to every engine \(see cmd/korrel8r/main.go\) alongside configuration rules, so a compiled rule takes effect as soon as it is linked in.
