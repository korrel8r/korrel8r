...
- Write a dedicated MCP server rather than generating from REST. Reasons:
  ...
  - Easier to deploy when built-in to korrel8r than as a separate proxy/bridge.
...
- Write custom MCP API instead of conversion from REST. Reasons:
  - Restrict MCP operations that the model can call.
  - Provide "you are a blah, you must do blah" type of prompts and descriptions, unsuitable in REST docs.
  - Use MCP features not available in a REST mapping - resources, prompts, others?
  - Expose new/different MCP tools that don't make sense on the REST API.

- Use AI to help with the rewrite?
- Find a way to re-use common documentation/definitions between all 3 APIs (CLI, MCP, REST)
- Drive from Red Hat light-speed integration.
