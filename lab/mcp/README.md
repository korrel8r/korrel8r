...
Large Language Models using [Korrel8r](https://korrel8r.github.io/korrel8r/) via the [Model Context Protocol](https://modelcontextprotocol.io/).
...
- `/mcp/sse` - MCP Server-Sent Events protocol
...
1. From OperatorHub, install the 'OpenShift Lightspeed Operator'
2. Configure lightspeed and a secret with your OpenAI API token.

   ```
   SECRET=<your-token-string> envsubst < credentials.yaml | oc apply -f -  -f olsconfig.yaml
   ```

3. Scale down the operator so you can manually edit the configmap.

   `kubectl scale --replicas=0 -n openshift-lightspeed deployment/lightspeed-operator-controller-manager`

4. Edit the config file and add the MCP section under `ols_config` (see olsconfig-cm-patch.yaml).

> **WARNING**: Incomplete, had problems with access quotas to OpenAI. To be continued.
>
>     oc logs -n openshift-lightspeed deployment/lightspeed-app-server
>
>     2025-07-22 18:19:03,002 [ols.app.endpoints.health:health.py:59] ERROR: LLM connection check failed with - Error code: 429 - ...
...
[openapi-mcp](https://github.com/jedisct1/openapi-mcp) is a tool that converts MCP to REST based on an OpenAPI spec.
...
This shell script runs an MCP stdio server (see `korrel8r-mcp-proxy.sh`).
...
- Hallucinates a graph out of nothing
